        ...
        options = [
            {
                ...
                "label": "按当前理解继续",
                "description": "确认系统基于需求文本和已配置资产形成的理解，可以作为后续设计范围。",
            },
            {
...
def _build_requirement_clarification_question(
    ...
    *,
    ...
    system_prompt = f"""You are a senior IT design requirement clarifier working in a ReAct style.
You must analyze the requirement together with uploaded materials and configured project assets before deciding the next clarification.
...
- On the first clarification round, you MUST ask exactly one choice-based question even if the requirement text is long.
- On later rounds, ask one more question only when a remaining ambiguity materially affects expert selection, design scope, interfaces, data, process, deployment, security, performance, or delivery boundaries.
...
- The question must be concrete and business-aware. Do not use stiff boilerplate such as "为了开始设计，请先明确...".
...
{{
  ...
  "context": {{
    "why_needed": "为什么这个问题会影响后续设计或专家选择",
    "options": [
      {{"value": "stable_option_id", "label": "选项名称", "description": "选这个选项意味着什么"}}
    ],
    "allow_free_text": true,
    "missing_information": ["scope_boundary"],
    "evidence_used": ["需求文本", "数据库表结构", "知识库命中", "代码仓目录"]
  }},
  "requirement_understanding": "当前对目标、范围、约束和资产线索的理解摘要",
  "assumptions": ["如果无需继续追问，写入可接受假设"]
    ...
    if not question:
        ...
        question = "基于当前材料，下面哪种理解最接近本次设计的真实范围？" if understanding else "下面哪种方向最接近本次需求需要优先澄清的范围？"
...
async def requirement_clarifier_node(state: DesignState) -> Dict[str, Any]:
    ...
    if question_payload:
        ...
        reasoning_sections = [
            REQUIREMENT_CLARIFIER_REASONING_TITLE,
            "",
            question_payload.get("reasoning") or "LLM 已结合需求材料和项目资产生成澄清问题。",
            "",
        ...
        return {
            ...
            "history": [
                "[系统] 需求澄清阶段已结合需求材料和项目资产完成分析，正在请求人工选择确认。",
            ],
    ...
    reasoning_sections = [
        REQUIREMENT_CLARIFIER_REASONING_TITLE,
        "",
        "LLM 已结合现有需求材料、人工回答和项目资产判断：当前信息足以进入规划阶段。",
    ]
    if clarification_summary and clarification_summary.get("summary"):

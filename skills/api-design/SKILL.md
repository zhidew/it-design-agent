---
name: api-design
description: 生成并校验项目级 API 设计产物，包含 HTTP API 设计与 RFC 9457 风格错误定义。支持根据受众（内部/外部/两者）自动判定并产出对应的 API 设计文件。
---
...
6. 如果缺少必需的输入参数，或者生成的产物无法满足输出产物 (Output Artifacts)，应直接终止并报错。
...
# 参考资料 (References)
- 阅读 `references/api-style-guide.md` 以获取具体的命名和设计规则。
- 复用 `assets/templates/api-internal.yaml`、`assets/templates/api-public.yaml` 与 `assets/templates/errors-rfc9457.json` 模板。

# 注意事项 (Notes)
- 无论是内部还是外部同步 HTTP API，都统一使用 API 设计表达；其核心差异体现在使用受众、暴露的路径范围、安全策略及生成的文件名上。

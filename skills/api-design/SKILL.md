...
- **专家边界**：只负责同步 API 契约、请求响应结构和错误模型；若涉及异步事件、补偿、重试编排，只做简短引用，不展开成 AsyncAPI 或集成编排详细设计。
- **依赖协同**：优先引用 `architecture-mapping`、`data-design`、`ddd-structure` 的边界和命名结果；不要在本专家产物里重新发明表结构、聚合边界、运维策略或测试计划。
...
```json
{
  ...
  "actions": [
    ...
    {"tool_name": "extract_lookup_values", "tool_input": {"files": ["baseline/original-requirements.md"]}}
  ],
